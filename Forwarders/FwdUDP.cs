...
{
    public class FwdUDP : ForwarderInterface
    {
        UdpClient output = null;
        Boolean active = true;
        ...
        public FwdUDP(BaseForwarder forwarder)
        {
        ...
        public void Start()
        {
            if (active)
            {
                if (!running)
                {
                    ...
                    output = new UdpClient(assignedForwarder.Parameters.ExtractedMetadata.GetElement("syslog"), port);
                }
                else
                {
                ...
                try
                {
                    if (!suspended)
                    {
                        if (output != null)
                        {
                            ...
                            output.Send(sendBytes,sendBytes.Length);
                            result = true;
                        }
                        else
                        {
