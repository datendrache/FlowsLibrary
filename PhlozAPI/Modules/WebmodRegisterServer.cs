...
namespace PhlozLib
{
    public class WebmodRegisterServer : ModuleInterface
    {
                        ...
                        if (sanitycheck)
                        {
                            ...
                            if (locatedAccount != null)
                            {
                                ...
                                if (BaseAccount.Authenticate(State.managementDB, username, pwHash))
                                {
                                    ...
                                    // Generate New Server Information

                                    Tree serverInfo = new Tree();

                                    string newServerName = FatumLib.GenerateRandomName();
                                    serverInfo.addElement("ServerName", newServerName);
                                    string newPassword = FatumLib.GenerateRandomPassword(128);  // Make them wish they were dead.
                                    serverInfo.addElement("ServerCred", FatumLib.Scramble(newPassword,""));

                                    string newServerXML = "";
                                    TextWriter outs = new StringWriter();
                                    TreeDataAccess.writeXML(outs, serverInfo, "ServerRequest");
                                    newServerXML = outs.ToString();

                                    msg = Encoding.ASCII.GetBytes(newServerXML);
                                }
                                else
                                {
